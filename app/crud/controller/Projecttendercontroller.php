...
namespace app\project_tender\controller;

use app\base\BaseController;
use app\project_tender\model\ProjectTender as ProjectTenderModel;
use app\project_tender\validate\ProjecttendercontrollerValidate;
use think\exception\ValidateException;
use think\facade\Db;
use think\facade\View;

class Projecttendercontroller extends BaseController
{
	/**
     * 构造函数
     */
    ...
    public function __construct()
    {
        ...
        $this->model = new ProjectTenderModel();
    }
    ...
    public function add()
    {
        ...
        if (request()->isAjax()) {		
            if (!empty($param['id']) && $param['id'] > 0) {
                try {
                    validate(ProjecttendercontrollerValidate::class)->scene('edit')->check($param);
                ...
                try {
                    validate(ProjecttendercontrollerValidate::class)->scene('add')->check($param);
                } catch (ValidateException $e) {
                    // 验证失败 输出错误信息
                    return to_assign(1, $e->getError());
                }
